      ...
      <div className="w-full max-w-[302px]">
        ...
        <form onSubmit={handleSubmit} className="space-y-0">
          ...
          <p className="text-center text-[13px] text-tertiary mt-[18px]">
            Nemáte účet?{" "}
            <Link to="/registrace" className="text-brand font-semibold">
              Zaregistrujte se
            </Link>
          </p>
        </form>
        ...
        <button
          onClick={async () => {
            ...
            setSubmitting(true);
            try {
              await login("test@cink.cz", "test1234");
            } catch (err: any) {
              setError(err.message);
            } finally {
              setSubmitting(false);
          ...
          disabled={submitting}
          className="w-full h-[44px] rounded-[13px] border border-border-light bg-chip text-[13px] font-semibold text-secondary hover:bg-border transition-colors mt-6 disabled:opacity-50"
        >
          Přihlásit jako test
        </button>
      </div>
